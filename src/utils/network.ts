...
function handleSpecialError(response: AxiosResponse): boolean {
  ...
  if (res.code === '210000' || res.code === '200000') {
    const $dialog = document.querySelector('.el-message-box__wrapper') as HTMLElement
  ...
  if (res.code === '-208999') {
    Message.error({ message: res.message, type: 'error' })
    return true
  }
  const path = router.currentRoute.path || ''
  if (path !== '/login' && res.code === '000005') {
    MessageBox.confirm('该账号已被停用，请联系管理员', '已被停用', {
  ...
  if (path !== '/login' && res.code === '000006') {
    MessageBox.confirm('该账号已被禁止登录，请联系管理员', '已被禁止登录', {
      confirmButtonText: '确认',
      type: 'warning'
    }).then(() => {
      router.push({ path: '/login' })
...
axios.interceptors.request.use(
  (config: AxiosRequestConfig) => {
    const headParams: object = JSON.parse(localStorage.getItem('userInfo') || '{}')
