      ...
      <div class="items-box" :key="index">
        <template v-for="(item,index) in flowItem.children">
          ...
          <template v-else-if="flowItem.type === flowTypeConstant.flow">
            <div class="node node-circle" @click="handleClick(item)" v-if="!item.hidden" :key="index">
              <div class="step-img" :class="item.className"></div>
            </div>
          </template>
          <template v-else-if="flowItem.type === flowTypeConstant.condition">
            <div class="node-square" @click="handleClick(item)" v-if="!item.hidden" :key="index">
              <div class="square">
                <div class="step-img" :class="item.className"></div>
              </div>
            </div>
...
<script>
  import {FLOW_ALL_LIST, FLOW_TYPE} from "../config"
